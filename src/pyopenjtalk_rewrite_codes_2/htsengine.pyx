...
#        cdef imageNet* c_tmp_net = imageNet.Create(len(args), c_lines)
        free(c_lines)
#        return HTS_Engine_generate_state_sequence_from_strings(self.engine, c_lines, num_lines) # HTS_Boolean 
        return res
    ...
    # C言語のFILE*をCPythonで使う方法がわからない。以下のエラーになる。
    # Cannot convert Python object argument to type 'FILE *'
    # 以下のようにしても同じ。
    # from libc.stdio cimport FILE
    # cdef FILE* FILE_PTR

#    def save_information(self, FILE * fp): HTS_Engine_save_information(self.engine, fp)
#    def save_information(self, FILE_PTR fp): HTS_Engine_save_information(self.engine, fp)

#    def save_label(self, FILE * fp): HTS_Engine_save_label(self.engine, fp)
#    def save_label(self, FILE_PTR fp): HTS_Engine_save_label(self.engine, fp)
...
#    def save_generated_speech(self, FILE_PTR fp): HTS_Engine_save_generated_speech(self.engine, fp)

#    def save_riff(self, FILE * fp): HTS_Engine_save_riff(self.engine, fp)
#    def save_riff(self, FILE_PTR fp): HTS_Engine_save_riff(self.engine, fp)

    # 以下URLを参考にしてファイルポインタを生成して渡す。
    # https://stackoverflow.com/questions/29950407/reading-and-writing-an-array-in-a-file-using-c-functions-in-cython
