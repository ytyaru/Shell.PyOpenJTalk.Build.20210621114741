...
#    if 0.0 <= weight_f0: _global_htsengine.set_gv_weight(1, weight) # https://cpp.hotexamples.com/jp/examples/-/-/FSCTerminate/cpp-fscterminate-function-examples.html
    ...
    if file_name is not None:
        with open(file_name, 'rb') as f: _global_htsengine.save_generated_speech(f)
    if info_file_name is not None:
        with open(info_file_name, 'rb') as f: _global_htsengine.save_information(f)
    if label_file_name is not None:
        with open(label_file_name, 'rb') as f: _global_htsengine.save_label(f)
    if param_file_name is not None:
        with open(param_file_name, 'rb') as f: _global_htsengine.save_generated_parameter(0, f) # stream_index
    if riff_file_name is not None:
        with open(riff_file_name, 'rb') as f: _global_htsengine.save_riff(f)
    """
...
#    if param_file_name is not None: _global_htsengine.save_generated_parameter(0, param_file_name) # stream_index
#    if riff_file_name is not None: _global_htsengine.save_riff(riff_file_name)
    ...
    """
    if file_name is not None and os.path.isfile(file_name): _global_htsengine.save_generated_speech(file_name.encode('utf-8'))
    if info_file_name is not None and os.path.isfile(info_file_name): _global_htsengine.save_information(info_file_name.encode('utf-8'))
    if label_file_name is not None and os.path.isfile(label_file_name): _global_htsengine.save_label(label_file_name.encode('utf-8'))
